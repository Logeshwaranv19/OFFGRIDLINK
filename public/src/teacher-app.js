...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (deleteAllQuizzesBtn) {
        deleteAllQuizzesBtn.addEventListener('click', async () => {
            if (!confirm(`Are you sure you want to delete ALL quizzes? This cannot be undone.`)) return;
            try {
                const result = await window.quizzesDB.allDocs({ include_docs: true });
                const quizzes = result.rows.map(r => r.doc).filter(d => d.type === 'quiz');
                const toDelete = quizzes.map(q => ({ ...q, _deleted: true }));
                
                if (toDelete.length > 0) {
                    await window.quizzesDB.bulkDocs(toDelete);
                    showToast('All quizzes deleted', 'info');
                    window.dispatchEvent(new CustomEvent('db-changed', { detail: { db: 'offgrid_quizzes' } }));
                    loadQuizzes();
                } else {
                    showToast('No quizzes to delete', 'info');
                }
            } catch (err) {
    ...
    async function loadResponses() {
        ...
        try {
            ...
            // Update leaderboard independently but trigger it here so it stays in sync
    ...
    async function updateScoreboardData() {
        try {
            const filter = document.getElementById('scoreboard-filter');
            const filterValue = filter ? filter.value : '';
            ...
            if (filterValue) {
                results = results.filter(r => r.quizId === filterValue);
            }

            loadLeaderboard(results);
        } catch (err) {
            console.error('[Teacher] Error updating scoreboard data:', err);
        }
    }

    // Expose for external calls (e.g. from peerjs)
    window.loadLeaderboard = updateScoreboardData;

    function loadLeaderboard(results) {
        const list = document.getElementById('leaderboard-list');
        if (!list) return;

        if (!results || results.length === 0) {
            list.innerHTML = `<div class="empty-state"><div class="icon">🏆</div><p>No scores yet.</p></div>`;
            if (window.broadcastLeaderboard) window.broadcastLeaderboard([]);
            return;
        }

        // Aggregate scores by student (studentId or studentName)
        const scoresMap = {};
        results.forEach(r => {
        ...
        const leaderboard = Object.values(scoresMap).sort((a, b) => b.totalScore - a.totalScore);
        
        list.innerHTML = leaderboard.map((entry, index) => {
            let color = 'var(--accent)';
            let medal = '';
            let bgStyle = 'background:var(--surface);';
            let borderStyle = 'border-bottom:1px solid var(--border);';
            let iconSize = '40px';
            let fontSize = '16px';
            ...
            if (index === 0) {
                color = '#eab308'; // Gold
                medal = '👑 ';
                bgStyle = 'background:rgba(234,179,8,0.05);';
                borderStyle = 'border:1px solid #eab308;';
                iconSize = '48px';
                fontSize = '20px';
            } else if (index === 1) {
                color = '#94a3b8'; // Silver
                ...
                color = '#b45309'; // Bronze
                medal = '🥉 ';
                bgStyle = 'background:rgba(180,83,9,0.05);';
                borderStyle = 'border:1px solid #b45309;';
                iconSize = '42px';
                fontSize = '17px';
            }
            
            return `
                <div style="display:flex;align-items:center;justify-content:space-between;padding:16px;${bgStyle}${borderStyle}margin-bottom:8px;border-radius:12px;box-shadow:var(--shadow-sm);transition:transform 0.2s;cursor:default;" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'">
                    <div style="display:flex;align-items:center;gap:16px;">
                        <div style="width:${iconSize};height:${iconSize};border-radius:50%;background:rgba(0,0,0,0.05);border:2px solid ${color};display:flex;align-items:center;justify-content:center;font-weight:800;font-size:${fontSize};color:${color};">${index + 1}</div>
                        <div>
                            <div style="font-weight:700;font-size:${fontSize};">${medal}${esc(entry.studentName)}</div>
                            <div style="font-size:12px;color:var(--text3);">${entry.quizzesTaken} Quizzes Taken</div>
                        </div>
                    </div>
                    <div style="font-weight:800;font-size:20px;color:var(--accent);">${entry.totalScore} <span style="font-size:12px;color:var(--text3);font-weight:600;">PTS</span></div>
                </div>
            `;
        }).join('');

        // Broadcast to students via P2P
        window.currentLeaderboardData = leaderboard;
        if (typeof window.broadcastLeaderboard === 'function') {
            window.broadcastLeaderboard(leaderboard);
        }
    }
    ...
    const deleteAllResponsesBtn = document.getElementById('delete-all-responses-btn');
    if (deleteAllResponsesBtn) {
        deleteAllResponsesBtn.addEventListener('click', async () => {
            ...
            try {
                // Delete all results
                const resResult = await window.resultsDB.allDocs({ include_docs: true });
                const results = resResult.rows.map(r => r.doc).filter(d => d.type === 'result');
                const resultsToDelete = results.map(r => ({ ...r, _deleted: true }));
                if (resultsToDelete.length > 0) {
                    await window.resultsDB.bulkDocs(resultsToDelete);
                }

                // Delete all submissions
                const subResult = await window.submissionsDB.allDocs({ include_docs: true });
                const submissions = subResult.rows.map(r => r.doc).filter(d => d.type === 'submission');
                const subsToDelete = submissions.map(s => ({ ...s, _deleted: true }));
                if (subsToDelete.length > 0) {
                    await window.submissionsDB.bulkDocs(subsToDelete);
                }

                if (resultsToDelete.length > 0 || subsToDelete.length > 0) {
                    showToast('All responses deleted successfully.', 'success');
                    window.dispatchEvent(new CustomEvent('db-changed', { detail: { db: 'offgrid_results' } }));
                    loadResponses();
                    updateScoreboardData();
                    updateStatsCount();
                } else {
                    showToast('No responses to delete.', 'info');
                }
            } catch (err) {
                console.error('[Teacher] Error deleting all responses:', err);
    ...
    async function updateResponseFilter() {
        ...
        const scoreSelect = document.getElementById('scoreboard-filter');
        if (!select && !scoreSelect) return;
        try {
            const result = await window.quizzesDB.allDocs({ include_docs: true });
            const quizzes = result.rows.map(r => r.doc).filter(d => d.type === 'quiz');
            
            const optionsHtml = quizzes.map(q => `<option value="${q._id}">${esc(q.title)}</option>`).join('');
            
            if (select) {
                select.innerHTML = '<option value="">All Quizzes</option>' + optionsHtml;
            }
            if (scoreSelect) {
                scoreSelect.innerHTML = '<option value="">All Quizzes (Total Score)</option>' + optionsHtml;
            }
        } catch (e) { console.error('[Teacher] Failed to update filters:', e); }
    }
    ...
    if (refreshScoreboardBtn) {
        refreshScoreboardBtn.addEventListener('click', () => {
            loadResponses();
            updateScoreboardData();
            showToast('Scoreboard refreshed', 'success');
        });
    }
    ...
    if (clearScoreboardBtn) {
        clearScoreboardBtn.addEventListener('click', async () => {
            if (!confirm('Are you sure you want to clear the scoreboard? This will delete all current student rankings (but keep quizzes intact).')) return;
            try {
                const results = await window.resultsDB.allDocs({ include_docs: true });
                const toDelete = results.rows
                    .map(r => r.doc)
                    .filter(d => d.type === 'result')
                    .map(d => ({ ...d, _deleted: true }));
                if (toDelete.length > 0) {
                    await window.resultsDB.bulkDocs(toDelete);
                }
                showToast('Scoreboard cleared!', 'success');
                loadResponses(); // This will clear the UI
                updateScoreboardData();
            } catch (err) {
                console.error('[Teacher] Error clearing scoreboard:', err);
                showToast('Failed to clear scoreboard', 'error');
            }
        });
    }
    ...
    // Scoreboard filter
    const scoreFilter = document.getElementById('scoreboard-filter');
    if (scoreFilter) scoreFilter.addEventListener('change', updateScoreboardData);
