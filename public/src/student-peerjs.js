...
(function () {
    ...
    async function handleTeacherMessage(data) {
        ...
        switch (data.type) {
            ...
            case 'submission_ack':
                ...
                if (data.result) {
                    try {
                        await window.resultsDB.put(data.result);
                        window.dispatchEvent(new CustomEvent('quiz-result-received', { detail: { result: data.result } }));
                    } catch (e) {
                        // ignore if already exists
                        if (e.name !== 'conflict') {
                            console.error('[Student PeerJS] Failed to save result:', e);
                        }
                    }
                }
                break;
            ...
            case 'leaderboard_update':
                console.log('[Student PeerJS] Leaderboard update received');
                window.dispatchEvent(new CustomEvent('leaderboard-updated', { detail: { leaderboard: data.data } }));
                break;
    ...
    // Expose a function to explicitly request leaderboard update
    window.requestLeaderboardUpdate = function(quizId = '') {
        if (teacherConn && teacherConn.open) {
            teacherConn.send({ type: 'leaderboard_request', quizId: quizId });
            showToast('Requesting latest scores...', 'info');
        } else {
            showToast('Connect to teacher to refresh scores', 'error');
        }
    };
