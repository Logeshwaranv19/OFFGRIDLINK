...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function submitQuiz(autoSubmit = false) {
        ...
        try {
            ...
            try {
                ...
                loadQuizList(); // Refresh the list UI
            } catch (e) {
            ...
            console.error('[Student] Error during quiz submission:', err);
    ...
    async function showResultForQuiz(quizId) {
        try {
            ...
            if (breakdown) {
                ...
                myResult.breakdown.forEach((b, idx) => {
                    const isCorrect = b.correct === true;
                    const isPending = b.correct === null;
                    const icon = isCorrect ? '<span style="color:var(--green)">✓</span>' : (isPending ? '<span style="color:var(--yellow)">⏱</span>' : '<span style="color:var(--red)">✗</span>');
                    
                    html += `
                        <div style="background:var(--surface2);border-radius:var(--radius-md);padding:12px;margin-bottom:10px;font-size:13px;">
                            <div style="font-weight:600;margin-bottom:6px;">Q${idx + 1}. ${esc(b.qText)}</div>
                            <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                                <span>Your answer:</span>
                                <span style="font-weight:600;color:${isCorrect ? 'var(--green)' : 'var(--text)'}">${esc(b.studentAnswer)}</span>
                            </div>
                            ${!isCorrect && !isPending ? `
                            <div style="display:flex;justify-content:space-between;">
                                <span>Correct answer:</span>
                                <span style="font-weight:600;color:var(--green)">${esc(b.correctAnswer)}</span>
                            </div>` : ''}
                            ${isPending ? `
                            ...
                            <div style="text-align:right;margin-top:8px;font-weight:700;font-size:12px;">
                                ${icon} ${isCorrect ? b.points : 0} / ${b.points} pts
                            </div>
                        </div>
                    `;
                });
    ...
    window.addEventListener('leaderboard-updated', (e) => {
        renderLeaderboard(e.detail.leaderboard);
    });
    ...
    function renderLeaderboard(leaderboard) {
        ...
        if (!leaderboard || leaderboard.length === 0) {
            list.innerHTML = `
                <div class="quizzes-empty">
                    <div class="empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg></div>
                    <h3>No scores yet</h3>
                    <p>Connect to teacher to see live rankings.</p>
                </div>`;
            return;
        }

        const myName = localStorage.getItem('student-name');
        ...
        list.innerHTML = leaderboard.map((entry, index) => {
            ...
            const isMe = myName && entry.studentName === myName;
            if (isMe) {
                bgStyle = 'background:rgba(0,0,0,0.05);';
                borderStyle = 'border:1px solid var(--accent);';
            }
            
            return `
                <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;${bgStyle}${borderStyle}margin-bottom:6px;border-radius:10px;box-shadow:var(--shadow-sm);gap:8px;">
                    <div style="display:flex;align-items:center;gap:10px;min-width:0;flex:1;">
                        <div style="width:${iconSize};height:${iconSize};border-radius:50%;background:rgba(0,0,0,0.05);border:2px solid ${color};display:flex;align-items:center;justify-content:center;font-weight:800;font-size:${fontSize};color:${color};flex-shrink:0;">${index + 1}</div>
                        <div style="min-width:0;flex:1;">
                            <div style="font-weight:700;font-size:${fontSize};${isMe ? 'color:var(--accent);' : ''} white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;">${medal}${esc(entry.studentName)} ${isMe ? '(You)' : ''}</div>
                            <div style="font-size:11px;color:var(--text3);">${entry.quizzesTaken} Quizzes</div>
                        </div>
                    </div>
                    <div style="font-weight:800;font-size:16px;color:var(--accent);flex-shrink:0;text-align:right;">${entry.totalScore} <span style="font-size:9px;color:var(--text3);font-weight:600;">PTS</span></div>
                </div>
            `;
        }).join('');
    }
    ...
    const studentRefreshBtn = document.getElementById('student-refresh-leaderboard');
    if (studentRefreshBtn) {
        studentRefreshBtn.addEventListener('click', () => {
            const filter = document.getElementById('student-scoreboard-filter');
            const quizId = filter ? filter.value : '';
            if (typeof window.requestLeaderboardUpdate === 'function') {
                window.requestLeaderboardUpdate(quizId);
            } else {
                showToast('Unable to request updates right now.', 'error');
            }
        });
    }

    const studentClearBtn = document.getElementById('student-clear-leaderboard');
    if (studentClearBtn) {
        studentClearBtn.addEventListener('click', () => {
            ...
            if (list) {
                list.innerHTML = `
                    <div class="quizzes-empty">
                        <div class="empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg></div>
                        <h3>Leaderboard cleared</h3>
                        <p>Click refresh to fetch live rankings.</p>
                    </div>`;
            }
        });
    }

    const studentScoreboardFilter = document.getElementById('student-scoreboard-filter');
    if (studentScoreboardFilter) {
        studentScoreboardFilter.addEventListener('change', () => {
            if (typeof window.requestLeaderboardUpdate === 'function') {
                window.requestLeaderboardUpdate(studentScoreboardFilter.value);
            }
        });
    }

    window.updateQuizListSelect = function(quizzes) {
        if (!studentScoreboardFilter) return;
        const currentVal = studentScoreboardFilter.value;
        studentScoreboardFilter.innerHTML = '<option value="">All Quizzes (Total Score)</option>' +
            quizzes.map(q => `<option value="${q._id}">${esc(q.title)}</option>`).join('');
        studentScoreboardFilter.value = currentVal;
    };

    window.addEventListener('quiz-result-received', (e) => {
        const result = e.detail.result;
        if (result && currentQuiz && result.quizId === currentQuiz._id) {
            // Update UI from local result to full result instantly
            showResultForQuiz(currentQuiz._id);
        } else if (result) {
            // Just notify that a grade was received
            showToast(`Grade received for ${result.quizTitle}!`, 'success');
        }
    });
