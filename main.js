...
function getLocalIP() {
    ...
    ips.sort((a, b) => {
        const isAWiFi = a.name.toLowerCase().includes('wi-fi') || a.name.toLowerCase().includes('wireless');
        const isBWiFi = b.name.toLowerCase().includes('wi-fi') || b.name.toLowerCase().includes('wireless');
        if (isAWiFi && !isBWiFi) return -1;
        if (!isAWiFi && isBWiFi) return 1;
        
        const isAVbox = a.address.startsWith('192.168.56.') || a.name.toLowerCase().includes('virtual') || a.name.toLowerCase().includes('veth');
        const isBVbox = b.address.startsWith('192.168.56.') || b.name.toLowerCase().includes('virtual') || b.name.toLowerCase().includes('veth');
        if (isAVbox && !isBVbox) return 1;
        if (!isAVbox && isBVbox) return -1;
...
function startFallbackHttpServer() {
    packageServer = http.createServer((req, res) => {
        ...
        if (req.method === 'GET' && req.url === '/api/local-ip') {
            ...
            res.end(JSON.stringify({ ip: currentIP }));
            return;
        }
        ...
        if (req.method === 'GET' && req.url === '/api/tracker-url') {
            const currentIP = getLocalIP();
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ url: `ws://${currentIP}:8000` }));
            return;
        }
        ...
        if (req.method === 'POST' && req.url === '/api/publish-quiz-package') {
            ...
            req.on('end', () => {
                try {
                    ...
                    const currentIP = getLocalIP();
                    const url = `http://${currentIP}:${FALLBACK_HTTP_PORT}/api/quiz-package/${encodeURIComponent(quizId)}`;
                    res.writeHead(200, { 'Content-Type': 'application/json' });
...
function createWindow() {
    ...
    mainWindow.webContents.on('did-finish-load', () => {
        ...
        mainWindow.webContents.send('local-ip', freshIP);
        console.log('[Main] Page loaded – sent local-ip:', freshIP);
    });
...
app.whenReady().then(async () => {
    ...
    try {
        ...
        console.error('[Main] Peer server error:', e);
    }
